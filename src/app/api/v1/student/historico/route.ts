...
// Método GET: Obtener el historico
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(
      "https://perfil.uagrm.edu.bo/estudiantes/show_page.php?opcion=2",
      { headers: headers }
    );
    ...
    contentTable.find("tr").each((index, row) => {
      ...
      const materiasData: MateriasInterface = {
        ...
        carrera: $(columns[1]).text().trim(),
        materia: $(columns[2]).text().trim(),
        cr: $(columns[3]).text().trim(),
        periodo: $(columns[4]).text().trim(),
        nota: $(columns[5]).text().trim(),
      };
