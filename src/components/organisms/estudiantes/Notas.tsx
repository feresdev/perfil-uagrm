'use client'
import { FiBook, FiInfo, FiNavigation, FiUser } from 'react-icons/fi';
import { ErrorResponse } from '@/types/errorResponse';
import { NotasMain } from '@/types/notas';
import formatFecha from '@/utils/common/FormatearFecha';

interface ComponentsProps {
    estado: { data: NotasMain | null, error: ErrorResponse | null , loading: boolean };
    popup: (active: boolean) => void
}

export default function NotasSection({ estado: { data, error, loading }, popup }: ComponentsProps) {
    return (
        <div>
                ...
                <div className='flex flex-col gap-4'>
                    ...
                    <div className='grid phone:grid-cols-1 desktop:grid-cols-5 tablet:grid-cols-2 w-full gap-4'>
                        ...
                        <div className='desktop:items-end justify-center flex flex-col gap-2 desktop:col-span-3'>
                            <div className={`px-3 bg-dark-main-color py-1 text-lg text-dark-title-color font-medium rounded-md flex items-center justify-start gap-1.5`}>
                                <span><FiBook size={'1.2rem'} /></span> {data.carrera}
                            </div>
                            <div className='flex gap-2'>
                                <div className={` truncate border-dark-border-alt-color phone:w-full rounded-md text-sm text-dark-text-color/90 bg-dark-container-alt-color border px-4 py-1.5`}>
                                    Constulta: {formatFecha(data.emision)}
                                </div>
                                <button onClick={() => popup(true)} className={`flex items-center justify-center gap-1 cursor-pointer border-blue-600 phone:w-full rounded-md text-sm text-dark-title-color bg-blue-600 border px-4 py-1.5`}>
                                    <span><FiNavigation size={'1rem'} /></span>Nueva consulta
                                </button>
                            </div>
                        </div>
                    </div>
                                            ...
                                            <table className="table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th className="py-3 px-4 text-left table-cell">SIGLA</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    {data && data.materias.map((materia, index) => (
                                                        <tr key={index + 1} className="relative hover:bg-dark-border-color transition-colors">
                                                            <td className="whitespace-nowrap">
                                                                <div className="flex">
                                                                    <span className="text-td">{materia.plan}</span>
                                                                </div>
                                                            </td>
                                                            <td className="whitespace-nowrap">
                                                            ...
                                                            <td className="whitespace-nowrap">
                                                                <div className="flex items-center gap-2">
                                                                    <span className="text-td capitalize">{materia.sigla}</span>
                                                                </div>
                                                            </td>
                                                            <td className="whitespace-nowrap">
                                                                <div className="flex items-center gap-2">
                                                            ...
                                                            <td className="whitespace-nowrap">
                                                                <div className={`${parseInt(materia.nota) >= 51 && 'bg-green-600'} ${parseInt(materia.nota) < 51 && 'bg-red-500'} ${materia.nota === 'INS' && 'bg-gray-600'} flex rounded-md px-4 py-1 -my-2 items-center justify-center`}>
                                                    ...
                                                    {!loading && data && data.materias.length === 0 && (
                                                        <tr>
                                                            <td colSpan={7}>
                                                                <nav className="flex items-center justify-between overflow-hidden">
                                                                    <div className="block">
                                                                        <p className="text-xs/tight text-td gap-1.5 flex items-center justify-start">
                                                                            <span><FiInfo size={'1rem'} /></span>
                                                                            No encontramos nigun resultado, le pedimos realizar una nueva consulta.
                                                                        </p>
                                                                    </div>
                                                                </nav>
                                                            </td>
